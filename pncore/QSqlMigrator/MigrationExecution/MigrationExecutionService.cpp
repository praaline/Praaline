                                        ...
                                        Direction direction) const
{
    ...
    if (migrationName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "migrationName is empty";
        return false;
    }

    const Migrations::Migration* migration = migrationContext.migrationMap()[migrationName];
    if (!migration) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "migration is missing: " << migrationName;
        return false;
    }

    CommandPtrList migrationCommands = migration->commandList();
    if (migrationCommands.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "no commands for migration";
        return false;
    }
    ...
    if (direction == Down) {
        ...
        if( migrationCommands.isEmpty() ) {
            ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "cannot reverse migration commands";
            return false;
        }
    }
    ...
    if (!migrationTableService) {
        ::qDebug() << QSQLMIGRATOR_LOG_PREFIX << Q_FUNC_INFO << "migrationTableService is 0";
        return false;
    }
    ...
    if (migrationTableService->canRevertStrucuturalChangesUsingTransactions()) {
        if (!database.transaction()) {
            ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "starting transaction failed";
            ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << database.lastError();
    ...
    ::qDebug() << QSQLMIGRATOR_LOG_PREFIX << "running Migration: " << migrationName;
