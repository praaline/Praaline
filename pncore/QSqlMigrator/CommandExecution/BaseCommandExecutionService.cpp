...
{
    ::qDebug() << QSQLMIGRATOR_LOG_PREFIX << "complete query-string looks like:";
    ::qDebug() << QSQLMIGRATOR_LOG_PREFIX << queryString;
    const QSqlQuery query = context.database().exec(queryString);
    const QSqlError error = query.lastError();
    if (error.isValid()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Error while executing query!";
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << error.text();
        return false;
    }
    return true;
}
