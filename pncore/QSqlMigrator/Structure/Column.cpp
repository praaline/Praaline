...
Column::Column(const QString &name, const SqlType &type, const QString &defaultValue, Attributes attributes)
    : m_name(name),
      m_type(type),
...
{
    if(name.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Column with empty name!";
    }
}

Column::Column(const QString &name, const SqlType &type, Attributes attributes)
    : m_name(name)
    , m_type(type)
    , m_attributes(attributes)
{
...
SqlType Column::type() const
{
    return m_type;
}

QString Column::defaultValue() const
{
    return m_defaultValue;
}

Column::Attributes Column::attributes() const
{
    return m_attributes;
}
