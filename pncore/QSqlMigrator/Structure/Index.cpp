      ...
      m_tableName(tableName)
{
    if(tableName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Index" << name << "with empty table name!";
    }
}
...
Index::Builder &Index::Builder::operator<<(const Index::Column &column)
{
    if( std::any_of( m_columns.begin(), m_columns.end(), [&column](const Index::Column& other)->bool { return other.name() == column.name(); } ) ) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Column is added twice to an index!" << m_name;
        return *this;
    }
    m_columns << column;
    return *this;
...
{
    if(name.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Index with empty name!" << tableName;
    }
    if(tableName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Index" << name << "with empty table name!";
    }
...
{
    if(name.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "Index with empty name!";
    }
}
