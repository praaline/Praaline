...
void MigrationRepository::storeMigration(const QString &migrationName, const Migration * migration)
{
    if(migrationName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "migrationName is empty. Can't store migration.";
        return;
    }

    if(!migration) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "migration is 0. Can't store migration.";
        return;
    }

    if(MigrationRepository::getInstance().m_migrationMap.contains(migrationName)) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << "migration" << migrationName << "already stored!";
        return;
    }
