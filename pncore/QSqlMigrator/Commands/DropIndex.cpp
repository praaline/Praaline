...
{
    if (name.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << DropIndex::typeName() << "command with empty name!";
    }
}
...
{
    if (!m_index.isValid()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << DropIndex::typeName() << "command with invalid index!";
    }
}
