...
DropColumn::DropColumn(const QString &columnName, const QString &tableName) :
    ...
    m_column(Structure::Column(columnName, Structure::SqlType::invalid()))
{
    if(columnName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << DropColumn::typeName()
    ...
    m_column(column)
{
    if(! column.isValid()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << DropColumn::typeName()
                     << "command with invalid column!";
    }
    if(tableName.isEmpty()) {
        ::qWarning() << QSQLMIGRATOR_LOG_PREFIX << DropColumn::typeName()
                     << "command" << column.name() << "with empty tableName!";
    }
}
