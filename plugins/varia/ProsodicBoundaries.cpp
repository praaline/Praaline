                                                 ...
                                                 QList<int> syllIndices)
{
    ...
    QSharedPointer<AnnotationTierGroup> tiers(corpus->repository()->annotations()->getTiers(annotationID, speakerID));

    IntervalTier *tier_syll = tiers->getIntervalTierByName("syll");
    if (!tier_syll) return results;
...
{
    QStringList results = analyseBoundaryListToStrings(corpus, annotationID, speakerID, syllIndices);
...
QStringList ProsodicBoundaries::analyseAnnotationToStrings(Corpus *corpus, const QString &annotationID)
{
    ...
    foreach (QString speakerID, tiersAll.keys()) {
        ...
        results << analyseBoundaryListToStrings(corpus, annotationID, speakerID, syllables);
    }
    qDeleteAll(tiersAll);
    return results;
}
