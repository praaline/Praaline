...
bool HTKForcedAligner::createFilesDCTandLAB(const QString &filenameBase, QList<AlignerToken> &atokens)
{
//    sil	sil
//    bon	b o~ sp
//    euh	eu sp
...
bool HTKForcedAligner::runAligner(const QString &filenameBase, QList<AlignerToken> &atokens, QList<AlignerPhone> &aphones)
{
    ...
    while (!rec.atEnd()) {
        ...
        aphones << AlignerPhone(phone, RealTime::fromNanoseconds(start), RealTime::fromNanoseconds(end), scoreAM, token);
    }
    return true;
}
...
void HTKForcedAligner::alignUtterances(QString waveFile, IntervalTier *tier_utterances, IntervalTier *tier_tokens, IntervalTier *tier_phones)
{
    ...
    QList<AlignerToken> atokens;
    QList<AlignerPhone> aphones;
    ...
    foreach (Interval *utt, list_utterances) {
        ...
        foreach (AlignerPhone phone, aphones) {
