...
Disfluency::Disfluency(IntervalTier *tierTokens, int start, int end, const QString &globalTag,
                       int interruptionPoint, int editTermStart, int editTermEnd, QObject *parent) :
    TokenSequence(tierTokens, start, end, globalTag, parent),
    m_interruptionPoint(interruptionPoint), m_editTermStart(editTermStart), m_editTermEnd(editTermEnd)
{
    if (!m_tierTokens) {
        // invalid tokens tier
        m_start = m_end = m_interruptionPoint = m_editTermStart = m_editTermEnd = -1;
        return;
    }
...
QList<Interval *> Disfluency::repairIntervals(int startFrom) const
{
    QList<Interval *> ret;
    if (!m_tierTokens) return ret;
    if (m_start < 0) return ret;
    if (m_interruptionPoint < 0) return ret;
    int start = (m_editTermEnd > 0) ? m_editTermEnd + 1 : m_interruptionPoint + 1;
...
QString Disfluency::repairText() const
{
    QString ret;
    foreach (Interval *intv, repairIntervals()) {
        ret.append(intv->text()).append(" ");
    }
    if (!ret.isEmpty()) ret.chop(1);
    return ret;
...
QList<QList<Interval *> > Disfluency::backtracking() const
{
    ...
    if (structureType() == Disfluency::Simple) {
        ...
        QList<Interval *> repair = repairIntervals();
        int i = 0, j = 0;
        while (i < reparandum.count() || j < repair.count()) {
