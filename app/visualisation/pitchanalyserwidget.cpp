...
#include "pitchanalyserwidget.h"

#include "framework/Document.h"
#include "framework/VersionTester.h"

#include "svcore/base/Debug.h"
...
PitchAnalyserWidget::PitchAnalyserWidget(bool withAudioOutput, bool withOSCSupport) :
    VisualiserWindowBase(withAudioOutput, false),
    m_overview(0),
...
PitchAnalyserWidget::~PitchAnalyserWidget()
{
    delete m_analyser;
...
void
PitchAnalyserWidget::setupMenus()
{
...
void
PitchAnalyserWidget::setupFileMenu()
{
...
void
PitchAnalyserWidget::setupEditMenu()
{
    if (m_mainMenusCreated) return;
...
void
PitchAnalyserWidget::setupViewMenu()
{
    if (m_mainMenusCreated) return;
...
void
PitchAnalyserWidget::setupAnalysisMenu()
{
    if (m_mainMenusCreated) return;
...
void
PitchAnalyserWidget::autoAnalysisToggled()
{
...
void
PitchAnalyserWidget::precisionAnalysisToggled()
{
    QAction *a = qobject_cast<QAction *>(sender());
    if (!a) return;
...
void
PitchAnalyserWidget::lowampAnalysisToggled()
{
    QAction *a = qobject_cast<QAction *>(sender());
    if (!a) return;
...
void
PitchAnalyserWidget::setupHelpMenu()
{
...
void
PitchAnalyserWidget::setupRecentFilesMenu()
{
...
void
PitchAnalyserWidget::setupToolbars()
{
...
void
PitchAnalyserWidget::moveOneNoteRight()
{
    // cerr << "VisualiserWidget::moveOneNoteRight" << endl;
    moveByOneNote(true, false);
}

void
PitchAnalyserWidget::moveOneNoteLeft()
{
    // cerr << "VisualiserWidget::moveOneNoteLeft" << endl;
    moveByOneNote(false, false);
}

void
PitchAnalyserWidget::selectOneNoteRight()
{
    moveByOneNote(true, true);
}

void
PitchAnalyserWidget::selectOneNoteLeft()
{
...
void
PitchAnalyserWidget::moveByOneNote(bool right, bool doSelect)
{
    int frame = m_viewManager->getPlaybackFrame();
    cerr << "VisualiserWidget::moveByOneNote startframe: " << frame << endl;
    ...
    Layer *layer = m_analyser->getLayer(PitchAnalyser::Notes);
    if (!layer) return;
    ...
    cerr << "VisualiserWidget::moveByOneNote endframe: " << frame << endl;
}

void
PitchAnalyserWidget::toolNavigateSelected()
{
    m_viewManager->setToolMode(ViewManager::NavigateMode);
...
void
PitchAnalyserWidget::toolEditSelected()
{
    cerr << "VisualiserWidget::toolEditSelected" << endl;
...
void
PitchAnalyserWidget::toolFreeEditSelected()
{
    m_viewManager->setToolMode(ViewManager::NoteEditMode);
...
void
PitchAnalyserWidget::updateMenuStates()
{
    ...
    bool haveWaveform =
        m_analyser->isVisible(PitchAnalyser::Audio) &&
        m_analyser->getLayer(PitchAnalyser::Audio);

    bool havePitchTrack =
        m_analyser->isVisible(PitchAnalyser::PitchTrack) &&
        m_analyser->getLayer(PitchAnalyser::PitchTrack);

    bool haveNotes =
        m_analyser->isVisible(PitchAnalyser::Notes) &&
        m_analyser->getLayer(PitchAnalyser::Notes);
...
void
PitchAnalyserWidget::showAudioToggled()
{
    m_analyser->toggleVisible(PitchAnalyser::Audio);

    QSettings settings;
    settings.beginGroup("MainWindow");

    bool playOn = false;
    if (m_analyser->isVisible(PitchAnalyser::Audio)) {
    ...
    m_analyser->setAudible(PitchAnalyser::Audio, playOn);
...
void
PitchAnalyserWidget::showPitchToggled()
{
    m_analyser->toggleVisible(PitchAnalyser::PitchTrack);

    QSettings settings;
    settings.beginGroup("MainWindow");

    bool playOn = false;
    if (m_analyser->isVisible(PitchAnalyser::PitchTrack)) {
        // just switched layer on; check whether playback was also on previously
        playOn = settings.value("playpitchwas", true).toBool();
    } else {
        settings.setValue("playpitchwas", m_playPitch->isChecked());
    }
    m_analyser->setAudible(PitchAnalyser::PitchTrack, playOn);
...
void
PitchAnalyserWidget::showSpectToggled()
{
    m_analyser->toggleVisible(PitchAnalyser::Spectrogram);
}

void
PitchAnalyserWidget::showNotesToggled()
{
    m_analyser->toggleVisible(PitchAnalyser::Notes);

    QSettings settings;
    settings.beginGroup("MainWindow");

    bool playOn = false;
    if (m_analyser->isVisible(PitchAnalyser::Notes)) {
        // just switched layer on; check whether playback was also on previously
        playOn = settings.value("playnoteswas", true).toBool();
    } else {
        settings.setValue("playnoteswas", m_playNotes->isChecked());
    }
    m_analyser->setAudible(PitchAnalyser::Notes, playOn);
...
void
PitchAnalyserWidget::playAudioToggled()
{
    m_analyser->toggleAudible(PitchAnalyser::Audio);
}

void
PitchAnalyserWidget::playPitchToggled()
{
    m_analyser->toggleAudible(PitchAnalyser::PitchTrack);
}

void
PitchAnalyserWidget::playNotesToggled()
{
    m_analyser->toggleAudible(PitchAnalyser::Notes);
}

void
PitchAnalyserWidget::updateLayerStatuses()
{
    m_showAudio->setChecked(m_analyser->isVisible(PitchAnalyser::Audio));
    m_showSpect->setChecked(m_analyser->isVisible(PitchAnalyser::Spectrogram));
    m_showPitch->setChecked(m_analyser->isVisible(PitchAnalyser::PitchTrack));
    m_showNotes->setChecked(m_analyser->isVisible(PitchAnalyser::Notes));
    m_playAudio->setChecked(m_analyser->isAudible(PitchAnalyser::Audio));
    m_playPitch->setChecked(m_analyser->isAudible(PitchAnalyser::PitchTrack));
    m_playNotes->setChecked(m_analyser->isAudible(PitchAnalyser::Notes));
}

void
PitchAnalyserWidget::editDisplayExtents()
{
    double min, max;
    double vmin = 0;
...
void
PitchAnalyserWidget::updateDescriptionLabel()
{
    // Nothing, we don't have one
}

void
PitchAnalyserWidget::documentModified()
{
    VisualiserWindowBase::documentModified();
}

void
PitchAnalyserWidget::documentRestored()
{
    VisualiserWindowBase::documentRestored();
}

void
PitchAnalyserWidget::newSession()
{
...
void
PitchAnalyserWidget::closeSession()
{
    if (!checkSaveModified()) return;
...
void
PitchAnalyserWidget::openLocation()
{
    QSettings settings;
    settings.beginGroup("MainWindow");
...
void
PitchAnalyserWidget::openRecentFile()
{
    QObject *obj = sender();
    QAction *action = qobject_cast<QAction *>(obj);

    if (!action) {
        cerr << "WARNING: VisualiserWidget::openRecentFile: sender is not an action"
             << endl;
        return;
    }
...
void
PitchAnalyserWidget::paneAdded(Pane *pane)
{
...
void
PitchAnalyserWidget::paneHidden(Pane *pane)
{
    if (m_overview) m_overview->unregisterView(pane);
}

void
PitchAnalyserWidget::paneAboutToBeDeleted(Pane *pane)
{
    if (m_overview) m_overview->unregisterView(pane);
}

void
PitchAnalyserWidget::paneDropAccepted(Pane *pane, QStringList uriList)
{
...
void
PitchAnalyserWidget::paneDropAccepted(Pane *pane, QString text)
{
    if (pane) m_paneStack->setCurrentPane(pane);
...
void
PitchAnalyserWidget::closeEvent(QCloseEvent *e)
{
...
bool
PitchAnalyserWidget::commitData(bool mayAskUser)
{
    if (mayAskUser) {
...
bool
PitchAnalyserWidget::checkSaveModified()
{
...
bool
PitchAnalyserWidget::waitForInitialAnalysis()
{
...
void
PitchAnalyserWidget::saveSession()
{
...
void
PitchAnalyserWidget::saveSessionInAudioPath()
{
...
void
PitchAnalyserWidget::saveSessionAs()
{
    // We do not want to save mid-analysis regions -- that would cause
    // confusion on reloading
    m_analyser->clearReAnalysis();
...
QString
PitchAnalyserWidget::exportToSVL(QString path, Layer *layer)
{
...
void
PitchAnalyserWidget::importPitchLayer()
{
...
PitchAnalyserWidget::FileOpenStatus
PitchAnalyserWidget::importPitchLayer(FileSource source)
{
    ...
    if (rdfType != RDFImporter::NotRDF) {
        ...
        try {
            ...
            if (model) {
                ...
                // cerr << "VisualiserWidget::importPitchLayer: Have model" << endl;
...
void
PitchAnalyserWidget::exportPitchLayer()
{
    Layer *layer = m_analyser->getLayer(PitchAnalyser::PitchTrack);
    if (!layer) return;
...
void
PitchAnalyserWidget::exportNoteLayer()
{
    Layer *layer = m_analyser->getLayer(PitchAnalyser::Notes);
    if (!layer) return;
...
void
PitchAnalyserWidget::doubleClickSelectInvoked(sv_frame_t frame)
{
    sv_frame_t f0, f1;
    m_analyser->getEnclosingSelectionScope(frame, f0, f1);

    cerr << "VisualiserWidget::doubleClickSelectInvoked(" << frame << "): [" << f0 << "," << f1 << "]" << endl;
...
void
PitchAnalyserWidget::abandonSelection()
{
    ...
    cerr << "VisualiserWidget::abandonSelection()" << endl;
...
void
PitchAnalyserWidget::selectionChangedByUser()
{
    if (!m_document) {
    ...
    cerr << "VisualiserWidget::selectionChangedByUser" << endl;

    m_analyser->showPitchCandidates(m_pendingConstraint.isConstrained());

    if (!selections.empty()) {
        Selection sel = *selections.begin();
        cerr << "VisualiserWidget::selectionChangedByUser: have selection" << endl;
    ...
    m_pendingConstraint = PitchAnalyser::FrequencyRange();
}

void
PitchAnalyserWidget::regionOutlined(QRect r)
{
    cerr << "VisualiserWidget::regionOutlined(" << r.x() << "," << r.y() << "," << r.width() << "," << r.height() << ")" << endl;

    Pane *pane = qobject_cast<Pane *>(sender());
    if (!pane) {
        cerr << "VisualiserWidget::regionOutlined: not sent by pane, ignoring" << endl;
        return;
    }

    if (!m_analyser) {
        cerr << "VisualiserWidget::regionOutlined: no analyser, ignoring" << endl;
        return;
    }

    SpectrogramLayer *spectrogram = qobject_cast<SpectrogramLayer *>
        (m_analyser->getLayer(PitchAnalyser::Spectrogram));
    if (!spectrogram) {
        cerr << "VisualiserWidget::regionOutlined: no spectrogram layer, ignoring" << endl;
        return;
    }
    ...
    cerr << "VisualiserWidget::regionOutlined: frame " << f0 << " -> " << f1
         << ", frequency " << v0 << " -> " << v1 << endl;

    m_pendingConstraint = PitchAnalyser::FrequencyRange(v0, v1);
...
void
PitchAnalyserWidget::clearPitches()
{
    MultiSelection::SelectionList selections = m_viewManager->getSelections();
...
void
PitchAnalyserWidget::octaveShift(bool up)
{
    MultiSelection::SelectionList selections = m_viewManager->getSelections();
...
void
PitchAnalyserWidget::togglePitchCandidates()
{
...
void
PitchAnalyserWidget::switchPitchUp()
{
...
void
PitchAnalyserWidget::switchPitchDown()
{
    if (m_analyser->arePitchCandidatesShown()) {
...
void
PitchAnalyserWidget::snapNotesToPitches()
{
...
void
PitchAnalyserWidget::auxSnapNotes(Selection s)
{
    cerr << "in auxSnapNotes" << endl;
    FlexiNoteLayer *layer =
        qobject_cast<FlexiNoteLayer *>(m_analyser->getLayer(PitchAnalyser::Notes));
    if (!layer) return;
...
void
PitchAnalyserWidget::mergeNotes()
{
    FlexiNoteLayer *layer =
        qobject_cast<FlexiNoteLayer *>(m_analyser->getLayer(PitchAnalyser::Notes));
    if (!layer) return;
...
void
PitchAnalyserWidget::deleteNotes()
{
    FlexiNoteLayer *layer =
        qobject_cast<FlexiNoteLayer *>(m_analyser->getLayer(PitchAnalyser::Notes));
    if (!layer) return;
...
void
PitchAnalyserWidget::formNoteFromSelection()
{
    Layer *layer0 = m_analyser->getLayer(PitchAnalyser::Notes);
...
void
PitchAnalyserWidget::playSpeedChanged(int position)
{
...
void
PitchAnalyserWidget::playSharpenToggled()
{
    QSettings settings;
    settings.beginGroup("MainWindow");
...
void
PitchAnalyserWidget::playMonoToggled()
{
    QSettings settings;
    settings.beginGroup("MainWindow");
...
void
PitchAnalyserWidget::speedUpPlayback()
{
...
void
PitchAnalyserWidget::slowDownPlayback()
{
    int value = m_playSpeed->value();
    value = value - m_playSpeed->pageStep();
...
void
PitchAnalyserWidget::restoreNormalPlayback()
{
    m_playSpeed->setValue(m_playSpeed->defaultValue());
}

void
PitchAnalyserWidget::audioGainChanged(int position)
{
    ...
    m_analyser->setGain(PitchAnalyser::Audio, gain);

    updateMenuStates();
}

void
PitchAnalyserWidget::increaseAudioGain()
{
...
void
PitchAnalyserWidget::decreaseAudioGain()
{
    int value = m_gainAudio->value();
    value = value - m_gainAudio->pageStep();
...
void
PitchAnalyserWidget::restoreNormalAudioGain()
{
    m_gainAudio->setValue(m_gainAudio->defaultValue());
}

void
PitchAnalyserWidget::pitchGainChanged(int position)
{
    ...
    m_analyser->setGain(PitchAnalyser::PitchTrack, gain);

    updateMenuStates();
}

void
PitchAnalyserWidget::increasePitchGain()
{
...
void
PitchAnalyserWidget::decreasePitchGain()
{
    int value = m_gainPitch->value();
    value = value - m_gainPitch->pageStep();
...
void
PitchAnalyserWidget::restoreNormalPitchGain()
{
    m_gainPitch->setValue(m_gainPitch->defaultValue());
}

void
PitchAnalyserWidget::notesGainChanged(int position)
{
    ...
    m_analyser->setGain(PitchAnalyser::Notes, gain);

    updateMenuStates();
}

void
PitchAnalyserWidget::increaseNotesGain()
{
...
void
PitchAnalyserWidget::decreaseNotesGain()
{
    int value = m_gainNotes->value();
    value = value - m_gainNotes->pageStep();
...
void
PitchAnalyserWidget::restoreNormalNotesGain()
{
    m_gainNotes->setValue(m_gainNotes->defaultValue());
}

void
PitchAnalyserWidget::audioPanChanged(int position)
{
    ...
    m_analyser->setPan(PitchAnalyser::Audio, pan);

    updateMenuStates();
}

void
PitchAnalyserWidget::increaseAudioPan()
{
...
void
PitchAnalyserWidget::decreaseAudioPan()
{
    int value = m_panAudio->value();
    value = value - m_panAudio->pageStep();
...
void
PitchAnalyserWidget::restoreNormalAudioPan()
{
    m_panAudio->setValue(m_panAudio->defaultValue());
}

void
PitchAnalyserWidget::pitchPanChanged(int position)
{
    ...
    m_analyser->setPan(PitchAnalyser::PitchTrack, pan);

    updateMenuStates();
}

void
PitchAnalyserWidget::increasePitchPan()
{
...
void
PitchAnalyserWidget::decreasePitchPan()
{
    int value = m_panPitch->value();
    value = value - m_panPitch->pageStep();
...
void
PitchAnalyserWidget::restoreNormalPitchPan()
{
    m_panPitch->setValue(m_panPitch->defaultValue());
}

void
PitchAnalyserWidget::notesPanChanged(int position)
{
    ...
    m_analyser->setPan(PitchAnalyser::Notes, pan);

    updateMenuStates();
}

void
PitchAnalyserWidget::increaseNotesPan()
{
...
void
PitchAnalyserWidget::decreaseNotesPan()
{
    int value = m_panNotes->value();
    value = value - m_panNotes->pageStep();
...
void
PitchAnalyserWidget::restoreNormalNotesPan()
{
    m_panNotes->setValue(m_panNotes->defaultValue());
}

void
PitchAnalyserWidget::updateVisibleRangeDisplay(Pane *p) const
{
    if (!getMainModel() || !p) {
        return;
    }
...
void
PitchAnalyserWidget::updatePositionStatusDisplays() const
{
...
void
PitchAnalyserWidget::outputLevelsChanged(float left, float right)
{
...
void
PitchAnalyserWidget::sampleRateMismatch(sv_samplerate_t requested, sv_samplerate_t actual,
                                     bool willResample)
{
...
void
PitchAnalyserWidget::audioOverloadPluginDisabled()
{
...
void
PitchAnalyserWidget::audioTimeStretchMultiChannelDisabled()
{
...
void
PitchAnalyserWidget::layerRemoved(Layer *layer)
{
    VisualiserWindowBase::layerRemoved(layer);
}

void
PitchAnalyserWidget::layerInAView(Layer *layer, bool inAView)
{
    VisualiserWindowBase::layerInAView(layer, inAView);
}

void
PitchAnalyserWidget::modelAdded(Model *model)
{
...
void
PitchAnalyserWidget::modelAboutToBeDeleted(Model *model)
{
    VisualiserWindowBase::modelAboutToBeDeleted(model);
}

void
PitchAnalyserWidget::mainModelChanged(WaveFileModel *model)
{
...
void
PitchAnalyserWidget::analyseNow()
{
...
void
PitchAnalyserWidget::analyseNewMainModel()
{
    WaveFileModel *model = getMainModel();

    cerr << "VisualiserWidget::analyseNewMainModel: main model is " << model << endl;
    ...
    if (pc < 2) {
        ...
        m_document->addLayerToView
            (selectionStrip,
             m_document->createMainModelLayer(LayerFactory::Type("TimeRuler")));
    } else {
...
void
PitchAnalyserWidget::modelGenerationFailed(QString transformName, QString message)
{
    if (message != "") {
...
void
PitchAnalyserWidget::modelGenerationWarning(QString transformName, QString message)
{
    QMessageBox::warning
...
void
PitchAnalyserWidget::modelRegenerationFailed(QString layerName,
...
void
PitchAnalyserWidget::modelRegenerationWarning(QString layerName,
                                     QString transformName, QString message)
{
    QMessageBox::warning
...
void
PitchAnalyserWidget::alignmentFailed(QString transformName, QString message)
{
    QMessageBox::warning
        (this,
...
void
PitchAnalyserWidget::rightButtonMenuRequested(Pane *pane, QPoint position)
{
...
void
PitchAnalyserWidget::handleOSCMessage(const OSCMessage &message)
{
    cerr << "VisualiserWidget::handleOSCMessage: Not implemented" << endl;
}

void
PitchAnalyserWidget::mouseEnteredWidget()
{
...
void
PitchAnalyserWidget::mouseLeftWidget()
{
    contextHelpChanged("");
}

void
PitchAnalyserWidget::website()
{
    //!!! todo: URL!
...
void
PitchAnalyserWidget::keyReference()
{
    m_keyReference->show();
}

//void
//VisualiserWidget::newerVersionAvailable(QString version)
//{
