...
PraalineSplash::PraalineSplash()
{
    setWindowFlags(windowFlags() | Qt::WindowStaysOnTopHint);

    QPixmap *p1 = new QPixmap(":icons/scalable/praaline-splash.png");
    ...
    if (widthMultiple > 2.5 || dpratio > 1.0) {
        // Hi-dpi either via pixel doubling or simply via lots of pixels
        ...
        QSvgRenderer renderer(QString(":icons/scalable/praaline-splash.svg"));
...
void PraalineSplash::finishSplash(QWidget *w)
{
    finish(w);
}

void PraalineSplash::drawContents(QPainter *painter)
{
    painter->drawPixmap(rect(), *m_pixmap, m_pixmap->rect());
    QString text = QString("v%1").arg(PRAALINE_VERSION);
    painter->drawText(width() - painter->fontMetrics().width(text) - (width()/50),
