...
QVariant NameValueListTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        switch (index.column()) {
        case 0: return d->nvl->displayString(index.row());  break;
        case 1: return d->nvl->value(index.row());          break;
        default:
            return QVariant();
        }
    }
...
bool NameValueListTableModel::insertRows(int row, int count, const QModelIndex &parent)
{
    if (d->nvl) return false;
...
bool NameValueListTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    if (d->nvl) return false;
